...
EMSCRIPTEN_KEEPALIVE
int get_number(cgame g, uint i, uint j) {
    return game_get_number(g, i, j);
}

EMSCRIPTEN_KEEPALIVE
bool is_empty(cgame g, uint i, uint j) {
    return game_is_empty(g, i, j);
}
...
EMSCRIPTEN_KEEPALIVE
int has_error(cgame g, uint i, uint j) {
